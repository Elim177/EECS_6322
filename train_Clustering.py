...
import Resnet_Model
import numpy as np
from Clustering import scan_train, calculate_total_loss, get_predictions
import os
...
for epoch in range(epoch):
        ...
        # Train
        print('***********Train********')
        scan_train(train_loader, model, criterion, optimizer, epoch, False)
        # Evaluate 
        ...
        torch.save({'model': model.module.state_dict(), 'head': lowest_loss_head}, 'scan_model')

# reload the last updated checkpoint
model_checkpoint = torch.load('scan_model', map_location='cpu')
